...
function normalizeGlobalModelConfig(config = {}) {
  const parallelTimeoutMs = Number(config.parallelTimeoutMs);
  return {
    ...
    parallelTimeoutMs: Number.isFinite(parallelTimeoutMs) && parallelTimeoutMs > 0 ? parallelTimeoutMs : 14_000,
  };
}
