...
function normalizeIpRateLimitRpm(value) {
  const number = Number(value ?? process.env.IP_RATE_LIMIT_RPM);
...
function normalizeGlobalModelConfig(config = {}) {
  ...
  return {
    ...
    ipRateLimitRpm: normalizeIpRateLimitRpm(config.ipRateLimitRpm),
  };
}
...
async function ensureProviderSchema() {
  try {
    ...
    await prisma.$executeRawUnsafe('ALTER TABLE "Provider" ADD COLUMN IF NOT EXISTS "contributor" TEXT');
    return true;
  } catch (error) {
...
async function ensureGlobalRouteController() {
  ...
  const created = await upsertProviderCompat({
    where: { name: GLOBAL_CONTROLLER_NAME },
    update: {},
    create: controller,
  });
...
async function getEnabledProviders() {
  ...
  const { enabledProviders } = await getRoutingConfig();
  return enabledProviders;
}

async function getGlobalRouteController() {
  await ensureGlobalRouteController();
  const { globalController } = await getRoutingConfig();
